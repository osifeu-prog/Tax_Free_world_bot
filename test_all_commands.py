...
async def test_command(session, command, chat_id):
    ...
    try:
        async with session.post(
            ...
            ok = data.get("ok")
            return {"command": command, "status": "✅" if ok else "❌", "time_ms": round(elapsed), "error": data.get("description", "") if not ok else ""}
    except Exception as e:
        return {"command": command, "status": "❌", "error": str(e), "time_ms": round((time.monotonic()-start)*1000)}

async def main():
    chat_id = int(sys.argv[1]) if len(sys.argv) > 1 else None
    if not chat_id:
        print("Usage: python test_all_commands.py <CHAT_ID>")
        return
