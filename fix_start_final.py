...
def main():
    ...
    # מחיקה מוחלטת של הקובץ הישן
    if os.path.exists(path):
        os.remove(path)
        print("🗑️  start.py הישן נמחק לחלוטין")
    
    # תוכן הקובץ - מחרוזת Raw לחלוטין
    content = r"""from aiogram import Router, F
...
"""
    ...
    with open(path, 'rb') as f:
        if f.read(3) == b'\xef\xbb\xbf':
            print("❌ BOM עדיין קיים!")
        else:
            print("✅ start.py נכתב בהצלחה - תקין וללא BOM")
