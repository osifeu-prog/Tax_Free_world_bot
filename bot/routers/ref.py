...
async def cmd_ref(msg: Message):
    ...
    await msg.answer(MESSAGES["ref_intro"].format(ref_code=code), parse_mode="HTML", reply_markup=back_to_start())
...
async def my_ref(call: CallbackQuery):
    ...
    await call.message.edit_text(MESSAGES["ref_intro"].format(ref_code=code), parse_mode="HTML", reply_markup=back_to_start())
    await call.answer()
