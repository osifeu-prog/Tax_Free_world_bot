...
async def cmd_stats(msg: Message):
    ...
    await msg.answer(MESSAGES["stats"].format(users=users, refs=refs, compares=compares), parse_mode="HTML", reply_markup=back_to_start())

@router.message(Command("top"))
async def cmd_top(msg: Message):
    leaders = await get_top_referrers(5)
    await msg.answer(MESSAGES["top_refs"].format(leaders=leaders), parse_mode="HTML", reply_markup=back_to_start())
