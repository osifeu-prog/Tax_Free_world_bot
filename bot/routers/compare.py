...
from bot.services.referral_service import log_command
...
async def cmd_compare(msg: Message):
    ...
    await msg.answer(text, parse_mode='HTML', reply_markup=share_result(amount, tx_per_month))
        await log_command(msg.from_user.id, 'compare', f'{amount} {tx_per_month}'); await save_user_memory(msg.from_user.id, 'compare', f'{amount} {tx_per_month}', text)
