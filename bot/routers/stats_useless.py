...
async def cmd_stats_useless(msg: Message):
    async with engine.begin() as conn:
        ...
        ai_replies = await conn.run_sync(lambda c: c.execute(text("SELECT COUNT(*) FROM useless_log WHERE action='ai_reply'")).fetchone()[0])
        roulette = await conn.run_sync(lambda c: c.execute(text("SELECT COUNT(*) FROM useless_log WHERE action='roulette_play'")).fetchone()[0])
        red_buttons = await conn.run_sync(lambda c: c.execute(text("SELECT COUNT(*) FROM useless_log WHERE action='red_button_pressed'")).fetchone()[0])
    text = (
